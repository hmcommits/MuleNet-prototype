...
export default function GraphVisualizer({
  ...
  return (
    <div
      className="relative rounded-2xl overflow-hidden border border-slate-800/80"
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
        <div
          className="flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest backdrop-blur-sm"
          style={{
            background: isAttackActive ? "rgba(220,20,60,0.15)" : "rgba(0,245,255,0.1)",
            border: isAttackActive ? "1px solid rgba(220,20,60,0.4)" : "1px solid rgba(0,245,255,0.3)",
        ...
        >
          ...
          />
          {isAttackActive ? "⚠ ATTACK DETECTED" : "H-GNN Monitor Active"}
        </div>
      </div>

      {/* Legend */}
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-1.5 backdrop-blur-sm">
        {[
          { color: "#A855F7", label: "Victim Account" },
          { color: "#DC143C", label: "Mule / Ghost Farm" },
          { color: "#FF8C00", label: "Suspicious Node" },
          { color: "#00F5FF", label: "Legitimate Account" },
        ].map(({ color, label }) => (
        ...
        <div className="border-t border-slate-800 mt-0.5 pt-1.5 flex items-center gap-2">
          <div className="w-2.5 h-2.5 rounded-sm" style={{ background: "#00F5FF22", border: "1px solid #00F5FF" }} />
          <span className="text-[9px] text-slate-400">Device Node (□)</span>
        </div>
        <div className="flex items-center gap-2">
          <svg width="10" height="10"><polygon points={hexPoints(5)} transform="translate(5,5)" fill="#00F5FF22" stroke="#00F5FF" strokeWidth="1" /></svg>
          <span className="text-[9px] text-slate-400">Subnet Node (⬡)</span>
        </div>
      </div>
